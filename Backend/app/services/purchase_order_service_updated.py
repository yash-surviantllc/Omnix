...
class PurchaseOrderService:  # Changed from ProductionOrderService
    ...
    async def list_purchase_orders(
        ...
        try:
            ...
            if search:
                search = f"%{search}%"
                query = query.or_(f"order_number.ilike.{search},product_name.ilike.{search}")
                        ...
                        try:
                            for item in order['items']:
                                try:
                                    ...
                                    print(f"Error processing item {item.get('id')} for order {order.get('id')}: {str(e)}")
                        except Exception as e:
                            print(f"Error fetching items for order {order.get('id')}: {str(e)}")
    ...
    def _derive_materials_status(db, order_id: str) -> str:
        ...
        try:
            materials_result = (
                ...
                .or_(f"order_id.eq.{order_id},purchase_order_id.eq.{order_id}")
                .execute()
            )
        except Exception:
        ...
        for mat in materials_result.data:
            status = mat.get('availability_status') or mat.get('status', '')
            if status in ['Available', 'Allocated']:
                available += 1
            if status in ['Shortage', 'Pending']:
                shortage += 1
    ...
    async def create_multi_sku_order(
        ...
        order_dict = {
            ...
            'product_code': primary_product.get('code'),
            'product_name': product_name,
    ...
    async def get_order_by_id(order_id: str) -> PurchaseOrderResponse:
        ...
        for mat in materials_result.data:
            ...
            materials.append(OrderMaterialResponse(
                ...
                total_cost=Decimal(str(mat.get('total_cost', 0))),
                status=mat.get('status', 'Pending'),
        ...
        return PurchaseOrderResponse(
            ...
            started_at=PurchaseOrderService._parse_datetime(order.get('started_at')) if order.get('started_at') else None,
            completed_at=PurchaseOrderService._parse_datetime(order.get('completed_at')) if order.get('completed_at') else None,
            created_at=PurchaseOrderService._parse_datetime(order.get('created_at')),
    ...
    async def update_order_status(
        ...
        existing = db.table('purchase_orders').select('id', 'status', 'started_at', 'completed_at').eq('id', order_id).execute()
        ...
        if status_data.status == 'In Progress' and not existing.data[0].get('started_at'):
    ...
    async def get_order_materials(order_id: str) -> List[MaterialRequirement]:
        ...
        for mat in materials_result.data:
            ...
            materials.append(MaterialRequirement(
                ...
                availability_status=mat.get('availability_status', 'Available')
            ))
    ...
    async def get_order_progress(order_id: str) -> OrderProgress:
        ...
        materials_result = db.table('order_materials').select('required_qty', 'allocated_qty', 'issued_qty').eq('order_id', order_id).execute()
    ...
    async def assign_team(order_id: str, user_ids: List[str], assigned_by: str) -> dict:
        ...
        for user_id in set(user_ids):
            ...
            }, on_conflict='order_id,user_id').execute()
    ...
    async def get_team_assignments(order_id: str) -> List[TeamAssignment]:
        ...
        assignments = db.table('order_team_assignments').select('*').eq('order_id', order_id).order('assigned_at', desc=False).execute()
