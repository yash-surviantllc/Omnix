...
SELECT
    ...
    'Sample purchase order #' || generate_series
...
BEGIN
    -- Get first order
    SELECT id INTO v_order_id FROM purchase_orders LIMIT 1;
...
CREATE INDEX IF NOT EXISTS idx_purchase_orders_customer ON purchase_orders(customer_name);
CREATE INDEX IF NOT EXISTS idx_purchase_orders_stage ON purchase_orders(production_stage);